  ...
  return (
    <>
      <section className="rounded-2xl border border-zinc-800/80 bg-zinc-900/60 p-8 shadow-[0_10px_40px_rgba(0,0,0,0.35)]">
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-amber-400/80">Monitoring</p>
        <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
          ...
          <button
            ...
            disabled={detecting}
            className="flex cursor-pointer flex-row items-center gap-2 rounded border border-amber-500/40 bg-amber-500/20 px-3 py-2 text-sm font-medium text-amber-200 hover:bg-amber-500/30 disabled:cursor-not-allowed disabled:opacity-50"
          >
            <i className="fa-solid fa-bolt"></i>
            {detecting ? 'Detecting...' : 'Detect Incidents'}
          </button>
        </div>
        <div className="mt-4 h-px w-28 bg-linear-to-r from-amber-400/80 to-transparent" />
                    ...
                    <details className="group">
                      <summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-4 py-3">
                        <div className="flex min-w-0 items-center gap-3">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="mt-1 text-xs text-zinc-400">Time Impact: {incident.timeImpact} sec</p>
                          </div>
                        </div>
                        <span className="rounded border border-amber-500/30 bg-amber-500/20 px-2 py-0.5 text-xs text-amber-200">
                          #{incident.id}
                        </span>
                      </summary>
                      ...
                      <div className="border-t border-zinc-800 px-4 py-3">
                        ...
                        <div className="mt-3 flex flex-wrap gap-3 text-xs text-zinc-400">
                          <span>Impact Count: {incident.impactCount}</span>
                        </div>
                          ...
                          {incident.pullRequest && pullRequestsById[incident.pullRequest] ? (
                            <>
                              ...
                              <div className="mt-1 text-zinc-400">
                                ...
                                {pullRequestsById[incident.pullRequest].base_branch}
                              </div>
                              {pullRequestsById[incident.pullRequest].body ? (
                                <div className="mt-3 rounded-md border border-zinc-800/80 bg-zinc-950/70 p-3">
                                  <div className="mb-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-zinc-500">
                                    Description
                                  </div>
                                  <div className="prose prose-invert max-w-none text-sm leading-7 prose-headings:text-zinc-100 prose-p:my-3 prose-p:text-zinc-300 prose-p:leading-7 prose-strong:text-zinc-100 prose-code:text-amber-200 prose-pre:border prose-pre:border-zinc-800 prose-pre:bg-zinc-900 prose-pre:text-sm prose-li:my-1 prose-li:text-zinc-300 prose-li:leading-7 prose-a:text-amber-300 hover:prose-a:text-amber-200">
                        ...
                        <div className="mt-3 flex flex-wrap gap-3 text-xs text-zinc-400">
                          {incident.pullRequest ? (
                            <button
                              ...
                              className="cursor-pointer flex flex-row items-center gap-2 rounded border border-amber-500/40 bg-amber-500/20 px-3 py-2 text-sm font-medium text-amber-200 hover:bg-amber-500/30 disabled:cursor-not-allowed disabled:opacity-50"
                            >
      ...
      {confirmDetect ? (
        <div onClick={() => setConfirmDetect(false)} className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4">
          <div onClick={(e) => e.stopPropagation()} className="w-full max-w-md rounded-xl border border-zinc-800 bg-zinc-900 p-5 shadow-[0_20px_60px_rgba(0,0,0,0.55)]">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-amber-400/80">Confirm Action</p>
            ...
            <div className="mt-5 flex items-center justify-end gap-2">
              ...
              <button
                ...
                className="flex cursor-pointer flex-row items-center gap-2 rounded border border-amber-500/40 bg-amber-500/20 px-3 py-1.5 text-sm font-medium text-amber-200 hover:bg-amber-500/30 disabled:cursor-not-allowed disabled:opacity-50"
              >
                <i className="fa-solid fa-bolt"></i>
        ...
        <div className="fixed inset-0 z-60 flex items-center justify-center bg-black/75 px-4">
          <div className="w-full max-w-md rounded-2xl border border-zinc-800 bg-zinc-900/95 p-6 shadow-[0_24px_80px_rgba(0,0,0,0.6)]">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-amber-400/80">
                  Detection Running
                </p>
            ...
            <div className="mt-5 flex items-center gap-2 text-amber-300">
              ...
              <span className="h-2 w-2 animate-bounce rounded-full bg-amber-300 [animation-delay:120ms]" />
          ...
          <div onClick={(e) => e.stopPropagation()} className="w-full max-w-md rounded-xl border border-zinc-800 bg-zinc-900 p-5 shadow-[0_20px_60px_rgba(0,0,0,0.55)]">
            ...
            <div className="mt-5 flex items-center justify-end gap-2">
              ...
              <button
                ...
                className="flex flex-row items-center cursor-pointer rounded border border-amber-500/40 bg-amber-500/20 px-3 py-1.5 text-sm font-medium text-amber-200 hover:bg-amber-500/30 disabled:cursor-not-allowed disabled:opacity-50"
              >
                <i className="fa-solid fa-code-pull-request"></i>
